...
interface OwnersTableProps {
  loading: boolean;
  advisers: any;
  deleteAdviser: (id: string | number) => void
}
export function AdvisersTable({loading, advisers, deleteAdviser}: OwnersTableProps) {
    ...
    <Box my={3}>
      ...
      <TableContainer>
        <Table>
          ...
          <TableBody>
            {!loading && advisers && advisers.length > 0 && advisers.slice(getCurrentPage(), getCurrentPageLimit()).map((row: any) => (
              <TableRow
                key={row.id}
                sx={{
              ...
              >
                ...
                <TableCell align='center'>
                  <Box display='flex'>
                    <IconButton onClick={() => router.push(`/admin/asesores-externos/${row.id}`)}>
                      <EditIcon/>
                    </IconButton>
                    <DeleteButton title='Se eliminara el siguiente asesor' element={row.first_name} onClick={() => deleteAdviser(row.id)}/>
                  </Box>
                </TableCell>
              </TableRow>
            ))}
      ...
      <Box sx={{display: 'flex', justifyContent: 'end', pt: 5}}>
        <Pagination
          boundaryCount={1}
          count={Math.ceil(advisers.length / 10)}
          defaultPage={1}
