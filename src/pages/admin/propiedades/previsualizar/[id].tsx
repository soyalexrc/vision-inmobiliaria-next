        ...
        {propertyData && !loading && (
          <>
            ...
            <Box my={4} display="flex" flexWrap="wrap" justifyContent="center">
              {propertyData?.images?.length < 1 && <Typography>No hay imagenes disponibles...</Typography>}
            ...
            <Divider sx={{ my: 2, borderWidth: '2px' }} />
            <Box>
              <Typography sx={{ mb: 3 }} variant="h5" color="secondary">
                Datos generales
              </Typography>
            ...
            <Box>
              ...
              <Grid container spacing={2}>
                {propertyData.attributes?.filter((x: any) => x.category === 'General').length > 0 &&
                  propertyData?.attributes
                    ?.filter((x: any) => x.category === 'General')
              ...
              <Divider sx={{ my: 2, borderWidth: '2px' }} />
              <Typography sx={{ mb: 3 }} variant="h5" color="secondary">
                Caracteristicas de la residencia
              </Typography>
              <Grid container spacing={2}>
                {propertyData.attributes.filter((x: any) => x.category === 'Property').length > 0 &&
                  propertyData?.attributes
                    ?.filter((x: any) => x.category === 'Property')
                    .map((item: any, index: number) => (
                      <Grid key={index + 1} item xs={12} sm={6} md={3}>
                        <Typography fontWeight="bold">{item.label}</Typography>
                        <Typography>{item.value}</Typography>
              ...
              <Grid container spacing={2}>
                {propertyData.attributes?.filter((x: any) => x.category === 'Furniture').length > 0 &&
                  propertyData.attributes
                    ?.filter((x: any) => x.category === 'Furniture')
                    .map((item: any, index: number) => (
                      <Grid key={index + 1} item xs={12} sm={6} md={3}>
                        <Typography fontWeight="bold">{item.label}</Typography>
                        <Typography>{item.value}</Typography>
              ...
              <Grid container spacing={2}>
                {propertyData.attributes.filter((x: any) => x.category === 'Custom').length > 0 &&
                  propertyData.attributes
                    ?.filter((x: any) => x.category === 'Custom')
                    .map((item: any, index: number) => (
                      <Grid key={index + 1} item xs={12} sm={6} md={3}>
                        <Typography fontWeight="bold">{item.label}</Typography>
                        <Typography>{item.value}</Typography>
