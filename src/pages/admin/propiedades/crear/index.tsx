...
import { Box, Typography, Button, useMediaQuery, Accordion, AccordionSummary, AccordionDetails, Icon, Grid } from '@mui/material';
      ...
      <>
        <Box display="flex" justifyContent="space-between" flexWrap="wrap" my={3}>
          ...
          <Button
            ...
            sx={{ mt: !largeScreen ? 2 : 0 }}
            onClick={handleGoToPrevisualize}
            variant="outlined"
            color="primary"
          >
            Vista previa
        ...
        <FormProvider {...methods}>
          <form>
            {panels.map((element) => (
              <Accordion key={element.id} expanded={expanded === element.state} onChange={handleChangePanel(element.state)}>
                <AccordionSummary
                ...
                >
                  <Box display="flex" alignItems="center">
                    <Icon sx={{ mr: 3, color: expanded === element.state ? '#fff' : 'secondary.main' }}>{element.icon}</Icon>
                    {/*<HandymanIcon sx={{mr: 2, color: expanded === element.state ? '#fff' : 'secondary.main'}}/>*/}
                    <Typography variant="h6" color={expanded === element.state ? '#fff' : 'secondary'}>
                      {element.title}
                    </Typography>
                  </Box>
                </AccordionSummary>
            ...
            <Grid container spacing={2} sx={{ mt: 3 }}>
              <Grid item xs={6}>
                <Button onClick={() => router.back()} fullWidth variant="outlined" color="primary">
                  Cancelar
                </Button>
              </Grid>
              <Grid item xs={6}>
                <Button disabled={loading} fullWidth onClick={onSubmit} variant="contained" color="primary">
                  {loading ? 'Registrando informacion' : 'Registrar propiedad'}
                </Button>
              </Grid>
            </Grid>
          </form>
