  ...
  async function getFormatsData() {
    try {
      ...
      if (response.status === 200) {
        ...
        const grouped = Array.from(groupBy(response.data, (format: FormatCashFlow) => format.entity)).map((element: any, index) => {
          return [element[0], sumTotals(element[1]), colors[element[0]]];
        });
      ...
      <Box>
        ...
        {temporalTransactions.map((transaction: any) => (
          <Paper elevation={2} sx={{ my: 3, p: 3 }} key={transaction.id}>
            <p>
              {' '}
              <b>ID de transaccion:</b> {transaction[0]}
            </p>
            <p>
              {' '}
              <b>Fecha de transaccion:</b> {moment(transaction[1][0].createdAt).calendar()}
            </p>
            <p>
              {' '}
              <b>Desde:</b> {transaction[1][0].entity}
            </p>
            <p>
              <b>Hasta: </b> {transaction[1][1].entity}
            </p>
            <p>
              <b>Monto: </b> {transaction[1][0].currency} {transaction[1][0].amount}
            </p>
            <p>
              <b>Responsable: </b> {transaction[1][1].createdBy}
            </p>
          </Paper>
        ))}
